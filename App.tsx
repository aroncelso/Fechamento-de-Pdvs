...
import { FinancialData, CalculationResult, AnalysisStatus, ClosingRecord, CloudStatus } from './types';
import { generateClosingReport } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleGenerateAIReport = async () => {
    ...
    try {
      const report = await generateClosingReport(data, results);
      setAnalysis(report);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Action Buttons */}
             <div className="flex flex-col sm:flex-row justify-end gap-3">
              <button
                ...
                className={`
                  flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-all hover:scale-[1.02] shadow-lg
              ...
              <button
                ...
                className={`
                  flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-[#0D0D0D] shadow-lg shadow-[#D9B68B]/20 transition-all
                  ${analysisStatus === AnalysisStatus.LOADING 
                    ? 'bg-slate-300 cursor-not-allowed text-slate-500' 
                    : 'bg-[#D9B68B] hover:bg-[#c9a67b] hover:scale-[1.02]'}
                `}
              >
                {analysisStatus === AnalysisStatus.LOADING ? (
                  <>
            ...
            {analysisStatus !== AnalysisStatus.IDLE && (
              <div className={`bg-white rounded-2xl border border-[#5A8C7A]/20 p-6 animate-fade-in transition-all duration-500 ${analysisStatus === AnalysisStatus.LOADING ? 'ring-2 ring-[#5A8C7A]/30' : ''}`}>
                <h3 className="text-lg font-bold text-[#025939] mb-4 flex items-center gap-2">
                  <Sparkles 
                    className={`text-[#D9B68B] transition-all duration-700 ${analysisStatus === AnalysisStatus.LOADING ? 'animate-pulse scale-110' : ''}`} 
                    size={20} 
                  />
                  Análise Gemini AI
                </h3>
                ...
                {analysisStatus === AnalysisStatus.LOADING ? (
                  <div className="space-y-4 animate-pulse">
                    <div className="h-4 bg-[#F2F2F2] rounded w-3/4"></div>
                    <div className="h-4 bg-[#F2F2F2] rounded w-full"></div>
                    <div className="h-4 bg-[#F2F2F2] rounded w-5/6"></div>
                    <div className="space-y-2 pt-2">
                       <div className="h-3 bg-[#F2F2F2] rounded w-full"></div>
                       <div className="h-3 bg-[#F2F2F2] rounded w-11/12"></div>
                       <div className="h-3 bg-[#F2F2F2] rounded w-4/5"></div>
                    </div>
                  </div>
                  ...
                  <div className="prose prose-slate max-w-none text-[#0D0D0D] prose-headings:text-[#025939] prose-strong:text-[#025939]">
